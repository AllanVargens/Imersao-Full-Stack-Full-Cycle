...
func (pd *ProductDB) GetProduct(id string) (*entity.Product, error) {
	var product entity.Product
	err := pd.db.QueryRow("SELECT * FROM products WHERE id = ?", id).Scan(&product.CategoryID, &product.ID, &product.Name, &product.Description, &product.Price, &product.ImageURL)
